...
export default function ContractExpireSoon(props: ContractExpireSoonProps) {
    ...
    return (
        <div class="flex flex-row items-center justify-between bg-background-muted/10 rounded-xl p-2 shadow-md">
            <div class="flex flex-col">
                <Heading components="h2" size="medium">{props.clientName} - {props.contractName}</Heading>
                <Text bold class="text-action-orange">Expire le {new Date(props.expireDate).toLocaleDateString("fr-FR")}</Text>
            </div>
